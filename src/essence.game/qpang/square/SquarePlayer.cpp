...
#include "packets/square/outgoing/MovePlayer.h"
#include "packets/square/outgoing/Chat.h"
#include "packets/square/outgoing/Emote.h"
#include "packets/square/outgoing/UpdatePlayerState.h"
#include "packets/square/outgoing/AddPlayer.h"
#include "packets/square/outgoing/UpdatePlayerEquipment.h"
...
void SquarePlayer::changeWeapon(uint32_t itemId)
{
	...
	m_square->sendPacket(UpdatePlayerEquipment(shared_from_this()));
}
...
void SquarePlayer::move(std::array<float, 3> position, uint8_t direction, uint8_t moveType)
{
	...
	m_square->sendPacket(MovePlayer(m_player->getId(), position, moveType, direction));
}

void SquarePlayer::chat(const std::u16string& message)
{
	m_square->sendPacket(Chat(m_player->getName(), message));
}

void SquarePlayer::emote(const uint32_t emoteId)
{
	m_square->sendPacketExcept(Emote(m_player->getId(), emoteId), m_player->getId());
}
...
void SquarePlayer::setState(uint32_t state, uint8_t stateValue)
{
	...
	m_square->sendPacket(UpdatePlayerState(shared_from_this(), m_stateValue));
}
